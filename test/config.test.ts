...
function messageStyle(override?: string): string {
  const env = { ...process.env, BOT_TOKEN: "123456:test-token" };
  delete env.MESSAGE_STYLE;
  if (override !== undefined) env.MESSAGE_STYLE = override;
  return execFileSync(
    process.execPath,
    ["--import", "tsx", "--input-type=module", "-e", 'import { config } from "./src/config.ts"; console.log(config.messageStyle)'],
    { env, encoding: "utf8" },
  ).trim();
}
...
test("Defaults preserve the requested Instagram/Reddit lists and FxEmbed", () => {
  ...
  assert.deepEqual(value.reddit, ["fxreddit.seria.moe", "rxddit.com", "vxreddit.com", "redditfix.com"]);
  assert.deepEqual(value.bilibili, ["fxbilibili.seria.moe", "vxbilibili.com"]);
  assert.deepEqual(value.threads, ["fixthreads.seria.moe", "vxthreads.net", "fixthreads.net"]);
  assert.deepEqual(value.facebookReel, ["fxfb.seria.moe", "facebed.seria.moe", "facebed.com"]);
...
test("Replace is the default message style", () => {
  assert.equal(messageStyle(), "replace");
  assert.equal(messageStyle("structured"), "structured");
});
